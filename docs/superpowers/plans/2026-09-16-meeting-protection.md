...
- **Scope:** this plan is Part 2 of the spec only, and has since merged to `main`. Part 1 was originally a separate "Close-to-Vault" close-and-reopen design, out of scope here — but per the spec's 2026-09-16 revision, that direction was superseded after the user clarified their actual requirement (same-tab in-place suspension with automatic restore-on-activate) matches the codebase's existing `strategy: "replace"` suspend mechanism, which is already implemented and tested. **Part 1 required no new code.**
