 ...
 *
 * `getUserMediaActive` is a distinct signal from `liveMediaTrackCount`: a
 * captured mic/camera stream isn't always attached to a local
 * `<video>`/`<audio>` element (e.g. a mic-only call whose audio is only ever
 * sent to a peer connection, never played back locally) — the DOM scan alone
 * would miss it, so a live getUserMedia grant confirms on its own.
 *
 * `rtcConnectionState: "disconnected"` is treated the same as
 * `"connecting"`/`"new"` (probable, not confirmed and not dropped to
 * none/possible) rather than as evidence the call ended: "disconnected" is
 * WebRTC's transient state for a network hiccup mid-call (ICE
 * renegotiating) and commonly self-heals within seconds — dropping
 * protection the instant it appears would suspend a tab mid-reconnect.
 *
 * @param {{ liveMediaTrackCount: number, screenShareActive: boolean, rtcConnectionState: string|null, isKnownMeetingDomain: boolean, getUserMediaActive?: boolean }} signals
...
export function deriveFrameLevel(signals) {
  ...
  if (liveMediaTrackCount > 0 || screenShareActive || getUserMediaActive || rtcConnectionState === "connected") {
    return "confirmed";
  }
  if (rtcConnectionState === "connecting" || rtcConnectionState === "new" || rtcConnectionState === "disconnected") {
    return "probable";
  }
...
/**
 * The single, fully pure guard every destructive suspend path (scheduled
 * sweep, manual suspend, bulk suspend, memory-pressure-accelerated timeout,
 * discard, and replace-strategy suspend all funnel through the same
 * `suspendTab()` in background.js, which calls this once at entry and again
 * immediately before the actual chrome.tabs API call) should evaluate
 * before touching a tab. Takes plain data — no chrome.* calls — precisely so
 * it can be unit tested against every level/domain combination without a
 * browser.
 *
 * @param {{
 *   neverSuspendInCall: boolean,
 *   level: string|null,
 ...
 *   exceptions: string[]
 * }} args
 * @returns {boolean} true if the tab must NOT be suspended/closed right now
 */
export function resolveTabProtection({
  neverSuspendInCall,
  level,
  lastReportedAt,
  now,
  ...
  if (!neverSuspendInCall) {
    // The user turned the whole protection off — the domain floor is part
    // of the same setting, so it's off too.
    return false;
  }
  const effectiveLevel = resolveEffectiveLevel({ lastLevel: level, lastReportedAt, now });
  return shouldProtectTab({ effectiveLevel, hostname, knownDomains, exceptions });
}
